...
class BERTCrossEncoder:
    ...
    def predict(self, query: str, passage: str) -> float:
        ...
        with torch.no_grad():
            ...
            score = torch.sigmoid(outputs.logits).item() if outputs.logits.shape[1] == 1 else outputs.logits.softmax(dim=1)[:, 1].item()
        return score
    ...
    def train(self, train_df: pd.DataFrame, val_df=None, epochs=3, batch_size=16, lr=2e-5):
        """TODO: Implement full training loop (pairwise / classification)"""
        print("🔧 Training function - silakan implementasikan full training loop di sini")
        pass
